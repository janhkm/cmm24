-- =============================================================================
-- Fix: search_listings - status Enum-Cast (listing_status -> TEXT)
-- Problem: "Returned type listing_status does not match expected type text"
-- Loesung: Expliziter Cast von l.status::TEXT und l.condition::TEXT im SELECT
-- =============================================================================

-- Bestehende Funktion droppen (noetig weil CREATE OR REPLACE den Return-Typ nicht aendern kann)
DROP FUNCTION IF EXISTS search_listings(TEXT, UUID, TEXT, INTEGER, INTEGER, INTEGER, INTEGER, TEXT, BOOLEAN, TEXT, INTEGER, INTEGER);
...
BEGIN
  ...
  -- Enum-Spalten (condition, status) werden explizit zu TEXT gecastet
  RETURN QUERY
  SELECT 
    l.id,
    l.account_id,
...
-- Berechtigungen erneut vergeben
