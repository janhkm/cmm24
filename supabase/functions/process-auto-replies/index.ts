// Supabase Edge Function: Process Auto-Reply Queue
// This function processes pending auto-replies and sends emails via Resend
// 
// Trigger: Cron job (every 5 minutes) or manual invocation
// URL: POST https://<project-ref>.supabase.co/functions/v1/process-auto-replies
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!resendApiKey) {
      console.warn('[AutoReply] RESEND_API_KEY not configured');
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: 'Email service not configured',
          result: { processed: 0, sent: 0, failed: 0, errors: ['RESEND_API_KEY missing'] }
        }),
    ...
    // Get pending auto-replies (max 10 per run to avoid timeout)
    const { data: pendingReplies, error: fetchError } = await supabase
      .rpc('get_pending_auto_replies', { p_limit: 10 });

    if (fetchError) {
      console.error('[AutoReply] Fetch error:', fetchError);
      return new Response(
    ...
    if (!pendingReplies || pendingReplies.length === 0) {
      console.log('[AutoReply] No pending replies');
      return new Response(
        JSON.stringify({ 
          success: true, 
    ...
    console.log(`[AutoReply] Processing ${pendingReplies.length} replies`);
    ...
    for (const reply of pendingReplies as PendingReply[]) {
      try {
        // Build email content
        let emailBody = buildEmailHtml(reply);
        ...
        if (emailResult.success) {
          ...
          result.sent++;
          console.log(`[AutoReply] Sent to ${reply.recipient_email}`);
        } else {
          // Mark as failed
          await supabase.rpc('mark_auto_reply_sent', {
            p_queue_id: reply.queue_id,
          ...
          console.error(`[AutoReply] Failed for ${reply.recipient_email}:`, emailResult.error);
        }
      } catch (error) {
        // Mark as failed
        ...
        console.error(`[AutoReply] Error for ${reply.recipient_email}:`, error);
      }
    }

    console.log(`[AutoReply] Completed: ${result.sent} sent, ${result.failed} failed`);
    ...
    console.error('[AutoReply] Unexpected error:', error);
    return new Response(
      JSON.stringify({ 
        success: false, 
...
function buildEmailHtml(reply: PendingReply): string {
  ...
  const messageHtml = reply.message
    .split('\n')
    .map(line => `<p style="margin: 4px 0; font-size: 14px; line-height: 24px; color: #333;">${line || '&nbsp;'}</p>`)
    .join('');
