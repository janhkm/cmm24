...
export async function createSellerAccount(companyName: string, phone?: string) {
  ...
  if (existing) {
    return { success: true as const, data: { accountId: existing.id, alreadyExists: true } };
  }
  ...
  if (error) {
    ...
    return { success: false as const, error: 'Fehler beim Erstellen des Verkaeuferkontos' };
  }
  
  return { success: true as const, data: { accountId, alreadyExists: false } };
}
