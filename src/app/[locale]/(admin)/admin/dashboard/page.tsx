...
function buildStats(data: { pendingListings: number; activeListings: number; totalAccounts: number; totalInquiries: number }) {
  return [
    {
      title: 'Zur Moderation',
      value: data.pendingListings.toString(),
...
export default function AdminDashboardPage() {
  const [stats, setStats] = useState(buildStats({ pendingListings: 0, activeListings: 0, totalAccounts: 0, totalInquiries: 0 }));
  // Pending-Listings-Details und Recent-Activity werden erst spaeter per API angebunden
  const pendingListings: { id: string; title: string; media: { url: string }[]; seller?: { companyName: string }; createdAt: string }[] = [];
  const recentActivity: { id: string; icon: typeof FileCheck; iconBg: string; iconColor: string; title: string; description: string; time: string }[] = [];
